    ...
    resource :cameras do
            ...
            point[point.length] = [
              {
                ...
                "properties": {
                  "marker-color": marker,
                  "Current Thumbnail Tag": "<img width='140' src='#{thumbnail}' />",
                  "Current Thumbnail URL": thumbnail,
                  "Camera Tag": "<a href='http://dash.evercam.io/v1/cameras/#{camera.exid}/live' target='_blank'>#{camera.name}</a>",
                  "Camera Name": camera.name,
                  "Camera ID": camera.exid,
                  "Data Processor": "Camba.tv Ltd\n\n01-5383333",
                  "Data Controller ID": camera.owner.username,
      ...
      patch '/:id' do
        ...
        if params[:is_public] and not caller.kind_of?(Client)
          IntercomEventsWorker.perform_async('made-camera-public', caller.email, caller.username)
        end
